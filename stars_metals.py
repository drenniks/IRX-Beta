...
f = open('star_metals.dat','w')
for i in numbers:
    data = np.genfromtxt('stars_' + str(i) + '.dat', names = ['x','y','z','h','mass','metals','age'])
    N = np.float(len(data))
    tot_metals = ((1/N) * np.float(np.sum(data['mass']*data['metals'])))/np.float(np.sum(data['mass']))
    f.write(str(i) + ' ' + str(tot_metals) + '\n')
