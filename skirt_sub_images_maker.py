...
for i in range(len(numbers)):
    copyfile('skirt_example_images.sh', 'skirt_images_' + str(numbers[i]) + '.sh')
    for line in fileinput.input('skirt_images_' + str(numbers[i]) + '.sh', inplace = 1):
        if 'RGB_wave_images_faceon_0.ski' in line:
            line = line.replace('RGB_wave_images_faceon_0.ski', 'RGB_wave_images_faceon_' + str(numbers[i]) + '.ski')
        sys.stdout.write(line)
        del line
        gc.collect()

    for line in fileinput.input('skirt_images_' + str(numbers[i]) + '.sh', inplace = 1):
        if 'RGB_wave_images_sideon_0.ski' in line:
            line = line.replace('RGB_wave_images_sideon_0.ski', 'RGB_wave_images_sideon_' + str(numbers[i]) + '.ski')
        sys.stdout.write(line)
        del line
        gc.collect()

    for line in fileinput.input('skirt_images_' + str(numbers[i]) + '.sh', inplace = 1):
        if 'IR_wave_images_faceon_0.ski' in line:
            line = line.replace('IR_wave_images_faceon_0.ski', 'IR_wave_images_faceon_' + str(numbers[i]) + '.ski')
        sys.stdout.write(line)
        del line
        gc.collect()

    for line in fileinput.input('skirt_images_' + str(numbers[i]) + '.sh', inplace = 1):
        if 'IR_wave_images_sideon_0.ski' in line:
            line = line.replace('IR_wave_images_sideon_0.ski', 'IR_wave_images_sideon_' + str(numbers[i]) + '.ski')
        sys.stdout.write(line)
        del line
        gc.collect()

    for line in fileinput.input('skirt_images_' + str(numbers[i]) + '.sh', inplace = 1):
        if 'multi_images' in line:
            line = line.replace('multi_images', 'multi_images_' + str(numbers[i]) + '.ski')
        sys.stdout.write(line)
        del line
        gc.collect()
