...
for i in range(len(numbers)):
    ...
    for line in fileinput.input('skirt_images_' + str(numbers[i]) + '.sh', inplace = 1):
        if 'single_wave_images_0.ski' in line:
            line = line.replace('single_wave_images_0.ski', 'single_wave_images_' + str(numbers[i]) + '.ski')
        sys.stdout.write(line)
        del line
        gc.collect()

    for line in fileinput.input('skirt_images_' + str(numbers[i]) + '.sh', inplace = 1):
        if 'multi_wave_images_0.ski' in line:
            line = line.replace('multi_wave_images_0.ski', 'multi_wave_images_' + str(numbers[i]) + '.ski')
        sys.stdout.write(line)
        del line
        gc.collect()
