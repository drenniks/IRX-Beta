#Writes the .sh files for each individual halo
import sys
import gc
import fileinput 
...
for i in range(len(numbers)):
    copyfile('skirt_example.sh', 'skirt_' + str(numbers[i]) + '.sh')
    for line in fileinput.input('skirt_' + str(numbers[i]) + '.sh', inplace = 1):
