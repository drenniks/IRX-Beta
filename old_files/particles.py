#writes particle number, used as test
...
f = open('particle_num.dat','w')
f.write('halo' + ' ' + '#stars' + ' ' + '#gas' + '\n')
for i in numbers:
    stars = np.genfromtxt('stars_' + str(i) + '.dat', names = ['x','y','z','h','mass','metals', 'age'])
    gas = np.genfromtxt('gas_' + str(i) + '.dat', names = ['x','y','z','h','mass','metals','temp'])

    part_stars = len(stars['x'])
    part_gas = len(gas['x'])
    
    f.write(str(i) + ' ' + str(part_stars) + ' ' + str(part_gas) + '\n')
